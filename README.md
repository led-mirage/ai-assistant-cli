...
- 標準入力（stdin）のパイプ入力に対応（ログや diff をそのまま AI に渡せる）
- パイプ入力は 最大 `stdin_max_bytes` まで読み取り（超過分は自動で切り詰め）
...
さらに短いエイリアスをつけると、キー入力のストレスを減らせます。
例えば次のように `~/.bashrc` に追加します。

```bash
# AI Assistant CLI
alias @='~/tools/ai-assistant-cli/ai.sh'
```
...
`-p` または位置引数、または stdin（パイプ入力）がある場合は、通常チャットモードとして動作します（`-1` を付けない限り）。

```bash
ai -p "自己紹介して"
ai "さっきの続き教えて"
cat error.log | ai 重要なエラーを抜き出して
```
...
- `-1 / --oneshot` を付けた場合
- 何もプロンプトを渡さずに実行した場合（`ai`）（ログインメッセージ用途）
...
AI Assistant CLI は、パイプやリダイレクトで渡された標準入力を読み取り、ユーザープロンプトに追加できます。

```bash
cat error.log | ai このエラーについて教えて
git diff | ai この変更の要点をレビューして
journalctl -xe | ai 重要そうなエラーを要約して
```

次のようにユーザープロンプトを省略した場合は、設定ファイルの `pipe_prompt` がプロンプトとして使用されます。
...
- 標準入力（stdin）のパイプ入力に対応
- 設定ファイルに stdin のみ入力時の補助プロンプト（`pipe_prompt`）を追加
- 設定ファイルに stdin 最大読み取りサイズの設定（`stdin_max_bytes`）を追加
